...
export default function CourseReportsPanel({ role }: { role: "ADMIN" | "TEACHER" }) {
  ...
  const load = useCallback(async (requestedPage: number) => {
    ...
    setError("");
    try {
      const response = await fetch(`/api/course-reports?page=${requestedPage}&pageSize=20`, { cache: "no-store" });
      const data = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(data?.error || "Không thể tải báo cáo.");
      setItems(data.items ?? []);
      setPage(data.page ?? requestedPage);
      setTotalPages(data.totalPages ?? 1);
      setResponses(Object.fromEntries((data.items ?? []).map((item: ReportItem) => [item.id, item.response ?? ""])));
    } catch (loadError) {
  ...
  useEffect(() => {
    let cancelled = false;
    fetch("/api/course-reports?page=1&pageSize=20", { cache: "no-store" })
      .then(async (response) => {
        const data = await response.json().catch(() => ({}));
        if (!response.ok) throw new Error(data?.error || "Không thể tải báo cáo.");
        if (cancelled) return;
  ...
  async function updateReport(item: ReportItem, status?: ReportItem["status"]) {
    setSavingId(item.id);
    setError("");
    const response = await fetch(`/api/course-reports/${item.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ response: responses[item.id] ?? "", status }),
    });
    const data = await response.json().catch(() => ({}));
    setSavingId(null);
    if (!response.ok) {
        ...
        {items.map((item) => (
          <article key={item.id} className="rounded-xl border border-slate-200 p-4">
            ...
            <label className="mt-4 block text-sm font-semibold text-slate-700">Phản hồi
              <textarea rows={3} value={responses[item.id] ?? ""} onChange={(event) => setResponses((current) => ({ ...current, [item.id]: event.target.value }))} className="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 font-normal" />
            </label>
            <div className="mt-3 flex flex-wrap gap-2">
              <button type="button" disabled={savingId === item.id} onClick={() => void updateReport(item, role === "TEACHER" && item.status === "PENDING" ? "IN_REVIEW" : undefined)} className="rounded-lg bg-blue-600 px-3 py-2 text-sm font-semibold text-white disabled:opacity-50">Lưu phản hồi</button>
              {role === "ADMIN" ? (
                <>
                  <button type="button" disabled={savingId === item.id} onClick={() => void updateReport(item, "IN_REVIEW")} className="rounded-lg border border-amber-200 px-3 py-2 text-sm font-semibold text-amber-700">Đang xem xét</button>
                  <button type="button" disabled={savingId === item.id} onClick={() => void updateReport(item, "RESOLVED")} className="rounded-lg border border-emerald-200 px-3 py-2 text-sm font-semibold text-emerald-700">Đã giải quyết</button>
                  <button type="button" disabled={savingId === item.id} onClick={() => void updateReport(item, "REJECTED")} className="rounded-lg border border-red-200 px-3 py-2 text-sm font-semibold text-red-700">Từ chối</button>
                </>
              ) : null}
            </div>
          </article>
