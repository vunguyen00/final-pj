    ...
    <main className="min-h-screen bg-slate-50 py-8">
      <div className="mx-auto max-w-7xl px-4">
        <section className="rounded-2xl border border-slate-200 bg-white p-6">
          ...
          <h1 className="mt-2 text-3xl font-bold text-slate-950">Bundle paths for language goals</h1>
        ...
        <section className="mt-6 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {displayed.map((course) => (
            <Link key={course.id} href={`/combos/${course.id}`} className="rounded-xl border border-slate-200 bg-white p-5 transition hover:border-blue-300 hover:shadow-lg">
              <div className="flex flex-wrap gap-2">
                ...
                <span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-bold text-slate-700">{getCourseLevel(course)}</span>
              </div>
              <h2 className="mt-4 text-xl font-bold text-slate-950">{course.name}</h2>
