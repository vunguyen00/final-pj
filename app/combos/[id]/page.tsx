    ...
    <main className="min-h-screen bg-slate-50">
      <section className="border-b border-slate-200 bg-white">
        <div className="mx-auto max-w-7xl px-4 py-10">
          <div className="flex flex-wrap gap-2">
            ...
            <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-bold text-slate-700">{getCourseLevel(combo)}</span>
            <span className="rounded-full bg-amber-50 px-3 py-1 text-xs font-bold text-amber-700">{getCertification(combo)}</span>
          </div>
          <h1 className="mt-5 text-4xl font-bold text-slate-950">{combo.name}</h1>
          <p className="mt-4 max-w-3xl text-lg leading-8 text-slate-600">{combo.description}</p>
          <div className="mt-8 flex flex-wrap gap-3">
            ...
            <Link href="/combos" className="rounded-lg border border-slate-200 px-5 py-3 font-semibold text-slate-700">All combos</Link>
          </div>
        </div>
      </section>
      ...
      <div className="mx-auto grid max-w-7xl gap-8 px-4 py-10 lg:grid-cols-3">
        <section className="space-y-4 lg:col-span-2">
          <h2 className="text-2xl font-bold text-slate-950">Bundle roadmap</h2>
        ...
        <aside className="rounded-xl border border-slate-200 bg-white p-5">
          <h2 className="text-xl font-bold text-slate-950">Bundle summary</h2>
          <dl className="mt-4 space-y-3 text-sm">
            <div className="flex justify-between"><dt className="text-slate-500">Teacher</dt><dd className="font-semibold text-slate-900">{combo.instructor?.username || "Teacher"}</dd></div>
            <div className="flex justify-between"><dt className="text-slate-500">Duration</dt><dd className="font-semibold text-slate-900">{getCourseDuration(combo)}</dd></div>
            <div className="flex justify-between"><dt className="text-slate-500">Lessons</dt><dd className="font-semibold text-slate-900">{totalLessons || combo.lessons}</dd></div>
            <div className="flex justify-between"><dt className="text-slate-500">Learners</dt><dd className="font-semibold text-slate-900">{combo._count.enrollments}</dd></div>
