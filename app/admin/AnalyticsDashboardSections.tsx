...
export type GrowthMode = "day" | "week" | "month" | "year";
...
const NUMBER_FORMATTERS: Record<0 | 1 | 2, Intl.NumberFormat> = {
  0: new Intl.NumberFormat("vi-VN", { minimumFractionDigits: 0, maximumFractionDigits: 0 }),
  1: new Intl.NumberFormat("vi-VN", { minimumFractionDigits: 1, maximumFractionDigits: 1 }),
...
function formatNumber(value: number, digits = 0) {
  const formatter = NUMBER_FORMATTERS[digits as 0 | 1 | 2] ?? NUMBER_FORMATTERS[0];
...
function KpiCard({
  ...
  return (
    <article className={`rounded-xl border border-slate-200/80 bg-gradient-to-br p-4 ${KPI_TONES[tone]}`}>
...
function DoughnutChart({ values }: { values: Array<{ label: string; value: number; color: string }> }) {
  ...
  return (
    <div className="flex flex-col items-center gap-4 md:flex-row md:items-start">
      <div className="relative h-44 w-44 rounded-full" style={{ background: `conic-gradient(${gradientStops.join(", ")})` }}>
        ...
        {values.flatMap((row, rowIndex) =>
          row.map((value, columnIndex) => (
            <div
              ...
              style={{ backgroundColor: `hsl(193 85% ${95 - value * 0.55}%)` }}
          ...
          {PRESETS.map((item) => (
            <button
              ...
              className={`rounded-lg px-3 py-2 text-sm font-medium transition ${
                preset === item.key ? "bg-slate-900 text-white shadow" : "border border-slate-200 bg-white text-slate-700 hover:bg-slate-50"
              }`}
            >
              {item.label}
            </button>
...
export function OverviewKpis({ data }: { data: AnalyticsPayload }) {
  return (
    <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      ...
      <KpiCard title="Tổng doanh thu" value={formatCurrency(data.overview.revenue.totalRevenue)} subtitle={`Admin: ${formatCurrency(data.overview.revenue.adminRevenue)} | GV: ${formatCurrency(data.overview.revenue.teacherRevenue)}`} tone="rose" />
    </section>
  );
}

export function GrowthAndRevenueSection({
  data,
  growthMode,
  growthSeries,
  onGrowthModeChange,
  ...
  return (
    <section className="grid gap-5 xl:grid-cols-2">
      ...
      <Panel title="Doanh thu theo thời gian" subtitle="Doanh thu khóa học và phân chia 70/30">
        <div className="mb-4 grid gap-3 sm:grid-cols-3">
          <KpiCard title="Admin nhận" value={formatCurrency(data.revenueAnalytics.adminRevenue)} tone="rose" />
          <KpiCard title="Giảng viên nhận" value={formatCurrency(data.revenueAnalytics.teacherRevenue)} tone="emerald" />
          <KpiCard title="Nạp ví" value={formatCurrency(data.revenueAnalytics.walletTopUpRevenue)} tone="sky" />
        </div>
...
export function CourseEnrollmentSection({ data }: { data: AnalyticsPayload }) {
  return (
    <section className="grid gap-5 xl:grid-cols-2">
      ...
      <Panel title="Enrollment Analytics" subtitle="Lượt ghi danh theo thời gian + top">
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-2">
          <div>
            ...
            <BarChart data={data.enrollmentAnalytics.topCourses.slice(0, 6).map((item) => ({ label: item.courseName, value: item.enrollments }))} color="bg-blue-500" />
          </div>
          <div>
            <p className="mb-2 text-sm font-semibold text-slate-700">Top giảng viên</p>
            <BarChart data={data.enrollmentAnalytics.topInstructors.slice(0, 6).map((item) => ({ label: item.instructorName, value: item.students }))} color="bg-indigo-500" />
          </div>
        </div>
      </Panel>
    </section>
...
export function TestAndAiSection({ data }: { data: AnalyticsPayload }) {
  return (
    <section className="grid gap-5 xl:grid-cols-2">
      <Panel title="Test Analytics" subtitle="Lượt làm bài, tỷ lệ đậu, điểm trung bình">
        ...
        <div className="mt-4">
          <SimpleTable
            ...
            rows={data.testAnalytics.topTests.slice(0, 5).map((item) => [item.name, formatNumber(item.attempts), formatNumber(item.averageScore, 2)])}
          />
        </div>
      </Panel>
      ...
      <Panel title="AI Assessment Analytics" subtitle="Type, band histogram, lỗi phổ biến">
        ...
        <div className="mt-4">
          ...
          <BarChart data={data.aiAnalytics.bandHistogram.map((item) => ({ label: `Band ${item.band}`, value: item.count }))} color="bg-violet-500" />
        </div>
        <div className="mt-4">
          <p className="mb-2 text-sm font-semibold text-slate-700">Điểm chấm AI trung bình theo tháng</p>
          <LineChart data={data.aiAnalytics.averageScoreByMonth} color="#7c3aed" />
        </div>
        <div className="mt-4 grid gap-3 md:grid-cols-2">
          <SimpleTable headers={["Top lỗi", "Số lần"]} rows={data.aiAnalytics.topMistakes.slice(0, 5).map((item) => [item.mistake, formatNumber(item.count)])} />
          <SimpleTable headers={["Tiêu chí bị trừ", "Điểm trừ"]} rows={data.aiAnalytics.topCriteriaPenalty.slice(0, 5).map((item) => [item.criteria, formatNumber(item.penalty, 2)])} />
        </div>
      </Panel>
    </section>
  );
...
export function GovernanceSection({ data }: { data: AnalyticsPayload }) {
  return (
    <section className="grid gap-5 xl:grid-cols-2">
      ...
      <Panel title="Anti Cheat Analytics" subtitle="Top hành vi và xu hướng theo thời gian">
        ...
        <div className="mt-4">
          <SimpleTable headers={["Hành vi", "Số lần"]} rows={data.antiCheatAnalytics.topBehaviors.slice(0, 6).map((item) => [item.name, formatNumber(item.value)])} />
        </div>
      </Panel>
    </section>
  );
...
export function LearningActivityPanel({ data }: { data: AnalyticsPayload }) {
  return (
    <Panel title="Learning Activity Heatmap" subtitle="Ngày học nhiều nhất, giờ cao điểm, hoạt động theo ngày">
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-3">
        ...
        <KpiCard
          title="Giờ cao điểm"
          value={data.learningActivityAnalytics.peakHour ? `${String(data.learningActivityAnalytics.peakHour.hour).padStart(2, "0")}:00` : "-"}
...
export function PointsLanguageSection({ data }: { data: AnalyticsPayload }) {
  return (
    <section className="grid gap-5 xl:grid-cols-3">
      ...
      <Panel title="Learning Language Analytics" subtitle="Phân bổ học viên, khóa học, bài thi">
  ...
  return (
    <Panel title="Export Reports" subtitle="Xuất snapshot hiện tại của dashboard">
      <div className="flex flex-wrap gap-3">
        <button type="button" onClick={onExportCsv} className="rounded-lg bg-slate-900 px-4 py-2 text-sm font-semibold text-white">
          Export CSV
        </button>
        <button type="button" onClick={onExportXlsx} className="rounded-lg bg-cyan-700 px-4 py-2 text-sm font-semibold text-white">
          Export XLSX
        </button>
        <button type="button" onClick={onExportPdf} className="rounded-lg bg-emerald-700 px-4 py-2 text-sm font-semibold text-white">
          Export PDF
        </button>
      </div>
    </Panel>
