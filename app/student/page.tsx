      ...
      <div className="mx-auto max-w-7xl space-y-8 px-4">
        <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <h1 className="mt-2 text-3xl font-bold text-slate-950 dark:text-white">Welcome back, {user.username}</h1>
              <p className="mt-2 max-w-2xl text-slate-600 dark:text-slate-300">
                Continue your language path, review upcoming tests, and discover the next course for your level.
              </p>
            </div>
            <div className="flex flex-wrap gap-3">
              <Link href="/student/lam-bai" className="rounded-lg bg-slate-950 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-800 dark:bg-white dark:text-slate-950">
                Practice now
              </Link>
              <Link href="/courses" className="rounded-lg border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-100 dark:border-slate-700 dark:text-slate-100 dark:hover:bg-slate-800">
                Browse courses
              </Link>
              <LogoutButton />
            </div>
        ...
        <section className="grid gap-6 lg:grid-cols-3">
          <div className="lg:col-span-2 rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
            ...
            <div className="mt-4 space-y-3">
              {activeCourses.slice(0, 4).map(({ course, progress, completed, totalLessons }) => (
                <Link key={course.id} href={`/student/hoc-bai?courseId=${course.id}`} className="block rounded-xl border border-slate-200 p-4 transition hover:border-blue-300 hover:bg-blue-50/40 dark:border-slate-800 dark:hover:bg-slate-800">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                      ...
                      <h3 className="mt-2 font-semibold text-slate-950 dark:text-white">{course.name}</h3>
                      <p className="mt-1 text-sm text-slate-500">{completed}/{totalLessons} lessons completed</p>
                    </div>
                    <span className="text-2xl font-bold text-slate-950 dark:text-white">{progress}%</span>
                  </div>
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
            <h2 className="text-lg font-bold text-slate-950 dark:text-white">Upcoming schedule</h2>
            <div className="mt-4 space-y-3">
              {tests.length > 0 ? tests.map((attempt) => (
                <div key={attempt.id} className="rounded-lg border border-slate-200 p-3 dark:border-slate-800">
                  <p className="font-medium text-slate-900 dark:text-white">{attempt.test.name}</p>
        ...
        <section className="grid gap-6 lg:grid-cols-2">
          <div className="rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
            <h2 className="text-lg font-bold text-slate-950 dark:text-white">My enrolled courses</h2>
            <div className="mt-4 grid gap-4 sm:grid-cols-2">
              {courseStats.map(({ course, progress }) => (
                <Link key={course.id} href={`/student/hoc-bai?courseId=${course.id}`} className="rounded-xl border border-slate-200 p-4 hover:border-blue-300 dark:border-slate-800">
                  <p className="text-xs font-semibold text-blue-600">{getCourseLanguage(course)} - {getCourseLevel(course)}</p>
                  <h3 className="mt-2 line-clamp-2 font-semibold text-slate-950 dark:text-white">{course.name}</h3>
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
            ...
            <div className="mt-4 space-y-3">
              {recommendedCourses.map((course) => (
                <Link key={course.id} href={`/courses/${course.id}`} className="flex items-center justify-between gap-4 rounded-xl border border-slate-200 p-4 hover:border-blue-300 dark:border-slate-800">
                  <div>
                    <p className="text-xs font-semibold text-blue-600">{getCourseLanguage(course)} - {getCourseType(course)}</p>
                    <h3 className="mt-1 font-semibold text-slate-950 dark:text-white">{course.name}</h3>
                    <p className="text-sm text-slate-500">{getCourseDuration(course)} - {course._count.enrollments} learners</p>
                  </div>
                  <span className="text-sm font-bold text-slate-950 dark:text-white">{priceLabel(course.price)}</span>
                </Link>
              ))}
        ...
        <section className="rounded-xl border border-slate-200 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
          <h2 className="text-lg font-bold text-slate-950 dark:text-white">Weekly analytics</h2>
          <div className="mt-4 grid gap-3 md:grid-cols-7">
            {["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map((day, index) => (
              <div key={day} className="rounded-lg bg-slate-100 p-3 dark:bg-slate-800">
                <div className="flex h-24 items-end">
                  <div className="w-full rounded bg-blue-600" style={{ height: `${20 + ((courseStats.length + index) % 5) * 14}%` }} />
                </div>
                <p className="mt-2 text-center text-xs font-medium text-slate-600 dark:text-slate-300">{day}</p>
              </div>
            ))}
          </div>
        </section>
