...
type StudentTakeTestClientProps = {
  testId: string;
  initialData: StudentTestPayload | null;
...
function createCaptionTrackSrc(text: string) {
  const cueText = toVttCueText(text) || "Audio prompt";
  const vtt = `WEBVTT\n\n00:00:00.000 --> 99:59:59.000\n${cueText}\n`;

  return `data:text/vtt;charset=utf-8,${encodeURIComponent(vtt)}`;
}

export default function StudentTakeTestClient({
  testId,
  initialData,
  initialError,
}: StudentTakeTestClientProps) {
  const router = useRouter();

  const test = initialData?.test ?? null;
    ...
    <main className="min-h-screen bg-slate-100 pb-12">
      ...
      <div
        className={`mx-auto px-4 py-6 sm:px-6 ${
          hasTestMaterial ? "max-w-[1600px]" : "max-w-6xl"
        }`}
      >
        {test.description ? (
        ...
        <div
          className={
            hasTestMaterial
              ? "grid items-start gap-6 lg:grid-cols-[minmax(0,1.05fr)_minmax(460px,0.95fr)]"
              : ""
          }
        >
          {hasTestMaterial ? (
            <aside className="lg:sticky lg:top-44 lg:max-h-[calc(100vh-12rem)] lg:overflow-y-auto">
              <TestMaterialPanel
                material={{
                  title: test.materialTitle,
                  content: test.materialContent,
                  url: test.materialUrl,
                  type: test.materialType,
                  data: test.materialData,
                }}
              />
            </aside>
          ) : null}
              ...
              {question.audioUrl ? (
                <div className="mt-4 rounded-xl bg-slate-50 p-3">
                  ...
                  >
                    ...
                    <track
                      kind="captions"
                      src={createCaptionTrackSrc(question.content)}
                      srcLang={test.language?.code || "vi"}
