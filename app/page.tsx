    ...
    <main className="min-h-screen bg-slate-50">
      <section className="border-b border-slate-200 bg-white">
        <div className="mx-auto grid max-w-7xl gap-10 px-4 py-16 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
          <div>
            ...
            <h1 className="mt-4 text-5xl font-bold tracking-tight text-slate-950">LearnHub</h1>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              ...
              <Link href="/student/tests" className="rounded-lg border border-slate-200 px-5 py-3 font-semibold text-slate-700 hover:bg-slate-100">Take placement test</Link>
            </div>
          </div>
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-5">
            <div className="grid gap-3 sm:grid-cols-2">
              {LANGUAGES.map((language, index) => (
                <Link key={language} href={`/courses?language=${language}`} className="rounded-xl border border-slate-200 bg-white p-5 hover:border-blue-300">
                  <p className="text-3xl font-bold text-slate-950">{["EN", "中文", "日本", "한글"][index]}</p>
                  <p className="mt-2 font-semibold text-slate-800">{language}</p>
      ...
      <section className="mx-auto max-w-7xl px-4 py-12">
        <div className="flex items-end justify-between gap-4">
          <div>
            <h2 className="text-2xl font-bold text-slate-950">Featured courses</h2>
            <p className="mt-1 text-slate-600">Popular paths across languages and product types.</p>
          </div>
      ...
      <section className="bg-white py-12">
        <div className="mx-auto grid max-w-7xl gap-8 px-4 lg:grid-cols-2">
          <div>
            <h2 className="text-2xl font-bold text-slate-950">Top teachers</h2>
            <div className="mt-5 grid gap-3 sm:grid-cols-2">
              {teachers.map((teacher) => (
                <div key={teacher} className="rounded-xl border border-slate-200 p-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-semibold text-slate-950">{teacher}</p>
          ...
          <div>
            <h2 className="text-2xl font-bold text-slate-950">Testimonials</h2>
            <div className="mt-5 space-y-3">
              {testimonials.map((text) => (
                <blockquote key={text} className="rounded-xl border border-slate-200 p-4 text-slate-600">
                  "{text}"
                </blockquote>
              ))}
            </div>
      ...
      <section className="border-t border-slate-200 bg-white py-12">
        <div className="mx-auto flex max-w-7xl flex-col gap-5 px-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 className="text-3xl font-bold text-slate-950">Start with the right language path</h2>
            <p className="mt-2 text-slate-600">Browse the marketplace or begin with a placement assessment.</p>
          </div>
          <Link href="/auth/register" className="rounded-lg bg-blue-600 px-5 py-3 font-semibold text-white hover:bg-blue-700">Create account</Link>
        </div>
      </section>
    </main>
  );
...
function CourseCard({ course }: { course: Awaited<ReturnType<typeof getHomeCourses>>[number] }) {
  return (
    <Link href={`/courses/${course.id}`} className="overflow-hidden rounded-xl border border-slate-200 bg-white transition hover:border-blue-300 hover:shadow-lg">
      ...
      <div className="p-4">
        <div className="flex flex-wrap gap-2">
          ...
          <span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-bold text-slate-700">{getCourseType(course)}</span>
        </div>
        <h3 className="mt-3 line-clamp-2 font-bold text-slate-950">{course.name}</h3>
