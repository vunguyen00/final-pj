...
const SEQUENCE_FLOWS = [
  ...
  },
  {
    ...
    messages: [
      ...
      ["api", "db", "Đọc User + TrustedDevice từ cookie đã băm"],
      ["api", "auth", "Xác minh scrypt, trạng thái, role và authVersion"],
      ["api", "db", "Thiết bị tin cậy: xóa Session cũ; tạo Session mới"],
      ["api", "db", "Thiết bị lạ: tạo LoginDeviceChallenge"],
      ["api", "mail", "Gửi liên kết xác nhận thiết bị lạ"],
      ["user", "ui", "Mở liên kết và xác nhận thiết bị"],
    ...
    notes: [
      ...
      [7, 3, 6, "Mỗi tài khoản chỉ có một Session hoạt động; thiết bị lạ phải xác nhận qua thư điện tử"],
      [18, 3, 5, "Đặt lại mật khẩu tăng authVersion và thu hồi toàn bộ Session cũ"],
    ],
  },
  {
  ...
  {
    name: "09 - Lời mời giảng viên",
    participants: [
      ["admin", "Quản trị viên", "actor"],
      ["ui", "Invitation UI", "ui"],
      ["api", "Invitation API", "api"],
      ["db", "PostgreSQL", "db"],
      ["mail", "SMTP / Email", "external"],
      ["teacher", "Người được mời", "actor"],
    ],
    messages: [
      ["admin", "ui", "Nhập email, tên và ngôn ngữ giảng dạy"],
      ["ui", "api", "POST /api/admin/invitations"],
      ["api", "db", "Kiểm tra User, LearningLanguage và lời mời còn hiệu lực"],
      ["api", "db", "Tạo UserInvitation với tokenHash + expiresAt"],
      ["api", "mail", "Gửi liên kết nhận lời mời"],
      ["teacher", "ui", "Mở /auth/accept-invitation?token=..."],
      ["ui", "api", "GET /api/auth/invitations"],
      ["api", "db", "Kiểm tra token chưa dùng/chưa thu hồi/còn hạn"],
      ["teacher", "ui", "Nhập và xác nhận mật khẩu mạnh"],
      ["ui", "api", "POST /api/auth/invitations"],
      ["api", "db", "Claim invitation trong giao dịch chống dùng lặp"],
      ["api", "db", "Tạo User TEACHER + TeacherApplication APPROVED + Log"],
      ["api", "db", "Tạo TrustedDevice + Session; gắn acceptedUserId"],
      ["api", "ui", "Đăng nhập và chuyển về trang chủ", "return"],
    ],
    notes: [
      [2, 2, 5, "Không tạo lời mời nếu email đã có tài khoản hoặc đang có lời mời còn hiệu lực"],
      [10, 2, 6, "Token thô chỉ nằm trong liên kết email; cơ sở dữ liệu chỉ lưu mã băm"],
    ],
  },
  {
    name: "10 - Đợt tuyển và thi giảng viên trực tiếp",
    participants: [
      ["admin", "Quản trị viên", "actor"],
      ["applicant", "Ứng viên", "actor"],
      ["ui", "Recruitment UI", "ui"],
      ["api", "Recruitment API", "api"],
      ["grader", "Giám khảo", "actor"],
      ["notify", "Email / Notification", "external"],
      ["db", "PostgreSQL", "db"],
    ],
    messages: [
      ["admin", "ui", "Tạo đợt tuyển, lịch và danh sách địa điểm"],
      ...
      ["admin", "api", "Mời dự thi hoặc từ chối hồ sơ"],
      ["api", "db", "Cập nhật INVITED_TO_EXAM/REJECTED + Notification"],
      ["api", "notify", "Gửi lịch, địa điểm thi hoặc lý do từ chối"],
      ["admin", "api", "Xuất liên kết chấm thi có token"],
      ["api", "db", "Lưu gradingTokenHash mới"],
      ["grader", "api", "Điểm danh, hoàn tất, nhập điểm 4 kỹ năng"],
      ["api", "db", "Upsert TeacherExamResult; PASSED hoặc REJECTED"],
      ["admin", "api", "Chuyển ứng viên đủ điểm thành TEACHER"],
      ["api", "db", "Đổi role, tăng authVersion, xóa Session + tạo Log"],
      ["api", "notify", "Gửi kết quả cuối cùng", "return"],
    ],
    notes: [
      [5, 2, 5, "Chỉ một đợt tuyển ở trạng thái OPEN; thời gian đăng ký phải kết thúc trước khi thi"],
      [13, 3, 6, "Bài thi diễn ra trực tiếp trên giấy; website quản lý hồ sơ, điểm danh và kết quả"],
    ],
  },
  {
    name: "11 - Quản trị viên xử lý hồ sơ giảng viên",
    participants: [
      ["admin", "Admin", "actor"],
      ["ui", "Admin UI", "ui"],
    ...
    messages: [
      ["admin", "ui", "Mở hồ sơ, chứng chỉ, đợt tuyển và kết quả thi"],
      ["ui", "api", "GET /api/admin/recruitment-rounds"],
      ["api", "db", "Đọc hồ sơ + TeacherExamResult + nhật ký"],
      ["api", "ui", "Chi tiết hồ sơ", "return"],
      ["admin", "ui", "Mời dự thi / từ chối / chuyển thành giảng viên"],
      ["ui", "api", "PATCH /recruitment-applications/{applicationId}"],
      ["api", "db", "Kiểm tra trạng thái, chứng chỉ và điểm trung bình 4 kỹ năng"],
      ["api", "db", "CONVERT: User.role=TEACHER, authVersion++, xóa Session"],
      ["api", "db", "Cập nhật TeacherApplication + tạo ApplicationLog"],
      ["api", "notify", "Tạo Notification"],
      ["notify", "mail", "Gửi email kết quả"],
      ["api", "ui", "Trạng thái mới", "return"],
    ],
    notes: [
      [6, 2, 5, "Chỉ hồ sơ PASSED có đủ điểm thi và đã nộp kết quả mới được chuyển thành TEACHER"],
    ],
  },
  {
...
const VI_MODEL_LABELS = {
  ...
  LoginDeviceChallenge: "Yêu cầu xác nhận thiết bị đăng nhập",
  UserInvitation: "Lời mời người dùng",
  ...
  RecruitmentRound: "Đợt tuyển giảng viên",
  TeacherExamResult: "Kết quả thi giảng viên trực tiếp",
...
const VI_FIELD_LABELS = {
  ...
  trustedDeviceId: "Mã thiết bị tin cậy",
  lastSeenAt: "Thời điểm hoạt động gần nhất",
  label: "Nhãn hiển thị",
  userAgent: "Thông tin trình duyệt và thiết bị",
  lastIp: "Địa chỉ IP gần nhất",
  verifiedAt: "Thời điểm xác minh",
  lastUsedAt: "Thời điểm sử dụng gần nhất",
  invitedById: "Mã người gửi lời mời",
  acceptedUserId: "Mã người dùng đã nhận lời mời",
  acceptedAt: "Thời điểm nhận lời mời",
  ...
  examLocationId: "Mã địa điểm thi",
  examLocationName: "Tên địa điểm thi",
  examLocationAddress: "Địa chỉ địa điểm thi",
  examLocationNote: "Ghi chú địa điểm thi",
  recruitmentRoundId: "Mã đợt tuyển",
  ...
  registrationOpensAt: "Thời điểm mở đăng ký",
  registrationClosesAt: "Thời điểm đóng đăng ký",
  examStartsAt: "Thời điểm bắt đầu thi",
  examEndsAt: "Thời điểm kết thúc thi",
  locations: "Danh sách địa điểm thi",
  gradingTokenHash: "Mã băm token chấm thi",
  gradingTokenIssuedAt: "Thời điểm cấp token chấm thi",
  createdById: "Mã người tạo",
  checkedIn: "Đã điểm danh",
  completed: "Đã hoàn thành bài thi",
  writingScore: "Điểm kỹ năng viết",
  speakingScore: "Điểm kỹ năng nói",
  listeningScore: "Điểm kỹ năng nghe",
  readingScore: "Điểm kỹ năng đọc",
  failed: "Không đạt",
...
const VI_ENUM_LABELS = {
  ...
  RecruitmentRoundStatus: "Trạng thái đợt tuyển giảng viên",
...
const ERD_DOMAINS = [
  ...
  {
    id: "teacher",
    name: "Tuyển dụng giảng viên và chống gian lận",
    color: "orange",
    models: [
      "TeacherApplication",
      "RecruitmentRound",
...
function renderErdOverview(models) {
  ...
  cells.push(
    vertex({
      ...
      value:
        `<b>FINNCENTER — SƠ ĐỒ QUAN HỆ CƠ SỞ DỮ LIỆU TỔNG QUAN (${models.length} BẢNG)</b>`,
      x: 40,
      y: 25,
      width: 1690,
      height: 52,
  ...
  return graphPage(
    `00 - Tổng quan ${models.length} bảng`,
    "erd-overview",
    cells,
    1780,
    Math.max(...columnY) + 80,
